...
export default function TopStoryDetailPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    if (id) fetchTopStories();
  }, [id]);

  if (!article) {
    return <p className="text-center text-gray-400 mt-10 flex justify-center">Loading...</p>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      {/* Back Button */}
      <button
        onClick={() => router.push("/topstories")}
      ...
      {article.url && (
        <a
          href={article.url}
          target="_blank"
          className="inline-block mt-8 px-4 py-2 bg-black/20 border rounded-md hover:bg-black/30"
        >
          Read Full Article →
        </a>
      )}
      ...
      <div className="flex justify-between mt-10">
        {hasPrev ? (
          <button
            onClick={() => router.push(`/topstories/${currentIndex - 1}`)}
